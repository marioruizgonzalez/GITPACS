create or replace procedure p_sku_instance_both
is
...
CURSOR C_SKU_INSTANCE(P_TRANSACTION_ID NUMBER, P_MAX_TRANSACTION_ID NUMBER) IS
SELECT A.ERP_INVENTORY_ID,
     ...
     FROM (SELECT MMT.TRANSACTION_ID ERP_INVENTORY_ID,
                  (SELECT UNIQUE MSI.SEGMENT1
                     FROM APPS.MTL_SYSTEM_ITEMS_B@of_iplnk MSI
                  ...
                  (SELECT UNIQUE TRANSACTION_TYPE_NAME
                     FROM INV.MTL_TRANSACTION_TYPES@of_iplnk MTTRX
                    WHERE     1 = 1
                  ...
                  (SELECT UNIQUE SLPN.LICENSE_PLATE_NUMBER
                     FROM APPS.WMS_LICENSE_PLATE_NUMBERS@of_iplnk SLPN
                  ...
                  (SELECT UNIQUE HLA.LOCATION_CODE
                     FROM APPS.HR_ALL_ORGANIZATION_UNITS@of_iplnk HAOU,
                          HR_LOCATIONS_ALL@of_iplnk HLA
                    WHERE     1 = 1
                  ...
                  (SELECT UNIQUE NVL (RCV.PROJECT_ID, 0)
                     FROM APPS.RCV_TRANSACTIONS@of_iplnk RCV,
                          APPS.PA_PROJECTS_ALL@of_iplnk PRO,
                          APPS.PA_TASKS@of_iplnk TSK
                    WHERE     1 = 1
                  ...
                  (SELECT UNIQUE FREIGHT_CODE
                     FROM APPS.WSH_CARRIERS@of_iplnk WSC,
                          APPS.WSH_DELIVERY_DETAILS@of_iplnk WDD
                    WHERE     1 = 1
                  ...
                  (SELECT UNIQUE
                     ...
                     FROM APPS.WSH_CARRIER_CONTACTS_V@of_iplnk WCCV,
                          APPS.WSH_DELIVERY_DETAILS@of_iplnk WDD
                    WHERE     1 = 1
                  ...
                  (SELECT UNIQUE WT.VEHICLE_NUMBER
                     FROM APPS.WSH_TRIPS@of_iplnk WT,
                          APPS.WSH_TRIP_STOPS@of_iplnk WTP,
                          APPS.WSH_TRIP_STOPS@of_iplnk WTD,
                          APPS.WSH_DELIVERY_LEGS@of_iplnk WDL,
                          APPS.WSH_DELIVERY_ASSIGNMENTS@of_iplnk WDA,
                          APPS.WSH_NEW_DELIVERIES@of_iplnk WND,
                          APPS.WSH_DELIVERY_DETAILS@of_iplnk WDD
                    WHERE     1 = 1
                  ...
                  (SELECT UNIQUE ORGANIZATION_CODE
                     FROM APPS.MTL_PARAMETERS@of_iplnk
                    WHERE     1 = 1
                  ...
                  (SELECT POH.SEGMENT1
                     FROM PO.PO_HEADERS_ALL@of_iplnk POH
                    WHERE     1 = 1
                          AND MMT.TRANSACTION_SOURCE_ID = POH.PO_HEADER_ID
                          AND MMT.TRANSACTION_SOURCE_TYPE_ID = NVL (NULL, 1))
                     OC,
          MMT.TRANSACTION_ID
             FROM INV.MTL_MATERIAL_TRANSACTIONS@of_iplnk MMT
            WHERE     1 = 1
                  AND MMT.TRANSACTION_ID >P_TRANSACTION_ID AND MMT.TRANSACTION_ID <= P_max_TRANSACTION_ID)
                  A;


		
CURSOR C_SKU_INSTANCE_DETAIL(P_TRANSACTION_ID NUMBER,P_MAX_TRANSACTION_ID NUMBER ) IS
SELECT /*+ FIRST_ROWS(10) */
          ...
          (SELECT DISTINCT CURRENT_STATUS 
					...
					WHERE MUT.TRANSACTION_ID = MSN.LAST_TRANSACTION_ID
					) STATUS_ID,
     ...
     FROM INV.MTL_UNIT_TRANSACTIONS@of_iplnk MUT,
          INV.MTL_TRANSACTION_LOT_NUMBERS@of_iplnk MTL,
          INV.MTL_MATERIAL_TRANSACTIONS@of_iplnk MMT
    WHERE     1 = 1
          AND MMT.TRANSACTION_ID >P_TRANSACTION_ID    AND MMT.TRANSACTION_ID<=P_MAX_TRANSACTION_ID
          AND MUT.TRANSACTION_ID >P_TRANSACTION_ID    AND MUT.TRANSACTION_ID<=P_MAX_TRANSACTION_ID
...
BEGIN
	...
	select max(TRANSACTION_ID)
        ...
        FROM INV.MTL_MATERIAL_TRANSACTIONS@of_iplnk;
...
EXCEPTION
  WHEN OTHERS THEN
    FOR indx IN 1 .. SQL%BULK_EXCEPTIONS.COUNT LOOP
			raise_application_error(-20001,v_TRANSACTION_ID || SQL%BULK_EXCEPTIONS(indx).ERROR_INDEX);
    END LOOP;
END;
...
BEGIN
  ...
  LOOP
    ...
    FORALL i IN 1 .. l_data_DETAIL.COUNT
      INSERT INTO GG_ADMIN.SKU_INSTANCE_DETAIL_HIST VALUES l_data_DETAIL (i);
    COMMIT;
...
BEGIN
  ...
  UPDATE GG_ADMIN.TBL_CONTROL_TRANSACTION_ID
     SET TRANSACTION_ID = P_MAX_TRANSACTION_ID,
         TABLE_ORIGIN = 2;
  commit;
