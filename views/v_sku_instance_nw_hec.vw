CREATE OR REPLACE FORCE VIEW GG_ADMIN.V_SKU_INSTANCE_NW_HEC AS
SELECT CASE
          ...
          os.transaction_source_id transaction_source_id,
     ...
     FROM (SELECT *
             FROM (SELECT enos.*
                     FROM ipacs.erp_nw_order_serials enos
                          INNER JOIN
                          (SELECT   t.serial_number, t.lot_number,
                                    t.inventory_item_id,
                                    MAX (t.creation_date) AS maxdate
                               FROM ipacs.erp_nw_order_serials t
                           GROUP BY t.serial_number,
                                    t.lot_number,
                                    t.inventory_item_id) tm
                          ON enos.inventory_item_id = tm.inventory_item_id
                        AND (   enos.serial_number = tm.serial_number
                             OR enos.serial_number = tm.serial_number
                            )
                        AND enos.creation_date = tm.maxdate
                          )) os
          LEFT JOIN
        ...
        AND osd.status IN ('Aprobada', 'Cerrado')
          ...
          (SELECT SID.serial_number, SID.lot_number, SID.inventory_item_id,
             ...
             FROM gg_admin.sku_instance_detail SID INNER JOIN gg_admin.sku_instance sih
        ...
        AND NVL (os.lot_number, os.serial_number) =
          ...
          (SELECT SID.serial_number, SID.lot_number, SID.inventory_item_id,
             ...
             FROM gg_admin.sku_instance_detail SID INNER JOIN gg_admin.sku_instance sih
                  ON SID.transacton_id = sih.erp_inventory_id
            WHERE 1 = 1 AND sih.transaction_type = 'PO Receipt') po_ski
        ...
        AND NVL (os.lot_number, os.serial_number) =
          ...
          LEFT JOIN
          (SELECT toh.status, tlm.movement_order_id, tlm.lpn
             FROM ipacs.snd_nw_transport_lpn_mapping tlm INNER JOIN ipacs.snd_nw_transport_order_headers toh
