CREATE OR REPLACE FORCE VIEW GG_ADMIN.V_TEST_1_6MILLON_V3 AS
SELECT
         A.ERP_INVENTORY_ID,
          A.SKU,
          A.ADD_DATE,
          ...
          A.OC,
          A.TRANSACTION_ID
     FROM (SELECT /*+ driving_site(MSI) */
                 MMT.TRANSACTION_ID ERP_INVENTORY_ID,
                  MSI.SEGMENT1 SKU,
                  ...
                  (SELECT /*+ driving_site(MIL) */
                    ...
                    WHERE     1 = 1
                          ...
                          AND ROWNUM = 1)
                    LOCATOR_SHORT_NAME,
                  (SELECT /*+ driving_site(MTST_ST) */
                     UNIQUE MTST_ST.TRANSACTION_SOURCE_TYPE_NAME
                  ...
                  (SELECT /*+ driving_site(SLPN) */
                     UNIQUE SLPN.LICENSE_PLATE_NUMBER
                  ...
                  (SELECT /*+ driving_site(FLV) */
                     UNIQUE NVL (FLV.MEANING, 'Def_Tx_Action')
                    ...
                    WHERE     1 = 1
                     ...
                     TRANSACTION_ACTION,
                  (SELECT /*+ driving_site(HAOU,HLA) */
                     UNIQUE HLA.LOCATION_CODE
                  ...
                  (SELECT /*+ driving_site(RCV,PRO,TSK) */
                     UNIQUE NVL (RCV.PROJECT_ID, 0)
                    ...
                    WHERE     1 = 1
                     ...
                     PROJECT_NUMBER,
                  (SELECT /*+ driving_site(RCV,PRO,TSK) */
                      UNIQUE NVL (RCV.TASK_ID, 0)                ---Number
                     FROM APPS.RCV_TRANSACTIONS@OFNX_SPERTO RCV,
                          APPS.PA_PROJECTS_ALL@OFNX_SPERTO PRO,
                          APPS.PA_TASKS@OFNX_SPERTO TSK
                    WHERE     1 = 1
                  ...
                  (SELECT /*+ driving_site(WSC,WDD) */
                    ...
                    WHERE     1 = 1
                     ...
                     FREIGHT_CODE1,
                  (SELECT /*+ driving_site(WSC,WDD) */
                      UNIQUE TO_CHAR (WSC.CARRIER_ID) CARRIER_ID
                     FROM APPS.WSH_CARRIERS@OFNX_SPERTO WSC,
                          APPS.WSH_DELIVERY_DETAILS@OFNX_SPERTO WDD
                    WHERE     1 = 1
                          AND WSC.CARRIER_ID = WDD.CARRIER_ID
                  ...
                  (SELECT /*+ driving_site(WCCV,WDD) */
                    ...
                    WHERE     1 = 1
                          ...
                          AND WDD.DELIVERY_DETAIL_ID = MMT.PICKING_LINE_ID)
                     NAME_DELIVERY_PERSON,
                  (SELECT /*+ driving_site(WT,WT,WTD,WDL,WDA,WND,WDD) */
                      UNIQUE WT.VEHICLE_NUMBER
                  ...
                  (SELECT  /*+ driving_site(HAOU,HLA) */
                    ...
                    WHERE     1 = 1
                          ...
                          AND HAOU.LOCATION_ID = HLA.LOCATION_ID)
                     POSTAL_CODE,
                  (SELECT /*+ driving_site(LMN) */
                      UNIQUE ORGANIZATION_CODE
                     FROM APPS.MTL_PARAMETERS@OFNX_SPERTO  LMN
                    WHERE     1 = 1
                          AND LMN.ORGANIZATION_ID = MMT.TRANSFER_ORGANIZATION_ID)
                     ORG_DES,
             ...
             FROM INV.MTL_MATERIAL_TRANSACTIONS@OFNX_SPERTO MMT,
                                                     INV.MTL_SYSTEM_ITEMS_B@OFNX_SPERTO MSI
            WHERE     1 = 1
                  AND MSI.ORGANIZATION_ID = MMT.ORGANIZATION_ID
                  AND MSI.INVENTORY_ITEM_ID = MMT.INVENTORY_ITEM_ID
                  AND MMT.TRANSACTION_ID >  2778219804
                  AND MMT.TRANSACTION_ID <= 2788219804) A
;
